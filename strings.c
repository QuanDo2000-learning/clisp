...
// Get lval with symbol k from the current environment.
...
// Put lval v with symbol k into the innermost environment.
...
lval* lval_call(lenv* e, lval* f, lval* a) {
  ...
  while (a->count) {
    // No more formal arguments to bind.
  ...
  // If '&' remains and binds to and empty list.
  ...
  if (f->formals->count == 0) {
    // All formal arguments are bound.
    f->env->par = e;
...
int main(int argc, char** argv) {
  ...
  // If supplied with list of files, load each file.
  if (argc >= 2) {
